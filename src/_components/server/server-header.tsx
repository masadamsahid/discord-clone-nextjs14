...
const ServerHeader = ({ server, role, ...props }: ServerHeaderProps) => {
      ...
      <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:text-neutral-400 space-y-[2px]">
        ...
        {isAdmin && (
          <DropdownMenuItem onClick={() => onOpen('members', { server })} className="px-3 py-2 text-sm cursor-pointer">
            Manage Members <Users className="size-4 ml-auto"/>
          </DropdownMenuItem>
        )}
        {isModerator && (
        ...
        {isAdmin && (
          <DropdownMenuItem onClick={() => onOpen('deleteServer', { server })} className="text-rose-500 hover:bg-rose-500 dark:hover:bg-rose-500 px-3 py-2 text-sm cursor-pointer">
            Delete Server <Trash className="size-4 ml-auto"/>
          </DropdownMenuItem>
        )}
        {!isAdmin && (
          <DropdownMenuItem onClick={() => onOpen('leaveServer', { server })} className="text-rose-500 hover:bg-rose-500 dark:hover:bg-rose-500 px-3 py-2 text-sm cursor-pointer">
